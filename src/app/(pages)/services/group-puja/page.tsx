...
export default function BrajYatraPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: "",
    email: "",
    puja: { value: "braj-yatra", label: "Braj Yatra" },
  });

  const description = `
Braj Yatra is a sacred pilgrimage that covers the divine land of Shri Krishna — 
Vrindavan, Mathura, Barsana, Nandgaon, Govardhan, and several spiritually powerful places.  
This holy journey brings peace, blessings, and divine connection with the pastimes 
of Lord Krishna and Radha Rani.  
Through our online booking facility, you can join or sponsor Braj Yatra easily 
and receive the blessings of Braj Dham directly from verified Braj Pandits.
  `;
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const bookingDetails = {
        ...
        trackingNumber: `TRK-${Math.random().toString(36).substring(2, 15)}`,
        puja: formData.puja,
        price: "FREE BOOKING", // ✔ price removed
      };
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-12 flex flex-col gap-12">
      ...
      {/* ⭐ HERO SECTION FOR BRAJ YATRA */}
      <section className="flex flex-col md:flex-row items-center gap-8 bg-yellow-100 p-6 rounded-lg shadow-md">
        
        {/* Left Text */}
        <div className="flex-1 flex flex-col gap-4">
          <h1 className="text-3xl font-bold text-primary">Braj Yatra</h1>
          ...
          <button
            onClick={() => setReadMore(!readMore)}
            className="text-primary font-medium underline mt-2"
          >
            {readMore ? "Read Less" : "Read More"}
          </button>
        </div>
      ...
      {/* ⭐ ABOUT SECTION */}
      <section className="flex flex-col gap-6">
        <h2 className="text-2xl font-semibold">About Braj Yatra</h2>
        <p className="text-gray-700 leading-relaxed">
          Braj Yatra is one of the holiest pilgrimages, covering Vrindavan, Mathura,
          Govardhan, Barsana, Nandgaon, and the sacred forests of Braj.  
          Devotees receive divine blessings, spiritual upliftment, and the grace of
          Radha Krishna by performing this Yatra.
        </p>
      </section>
      ...
      >
        ...
        {/* Right Form */}
        <div className="flex flex-col gap-6">
          <h2 className="text-2xl font-semibold mb-2">Book Braj Yatra</h2>
          <p className="text-gray-600 text-sm">
            Join or sponsor the divine Braj Yatra and receive blessings from the land of Krishna.
          </p>
